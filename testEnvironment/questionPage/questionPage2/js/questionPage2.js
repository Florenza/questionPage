...
$(function () {
    ...
    var money = void 0, loanTime,
    ID = void 0,
    workTime = void 0,
    jobType = void 0,
    qualifications = void 0,
    revenue = void 0, liveCity,
    result = void 0;
    ...
    function onload() {
        ...
        if (userTel == "" || userTel == undefined || userTel == null) {
            ...
            if (login != 1) {//判断是否有登录状态
                login = 1;//补录登录状态
                setCookie("login", login);
            }
            else {
                getLocationPlace(APITitle,userTel);//补录地区
              
                    $.ajax({
                        type: 'get',
                        url: APITitle + "api/Questionnaire/CheckAnswerByQNID/2?mobileNum=" + userTel,
                        data: {},
                        success: function success(data) {
                            console.log(data);
                            if (data.length>0) {
                                ...
                                for (var i = 0; i < restData.length; i++) {
                                    ...
                                    var pQualifications = qualifications;
...
function getLocationPlace(APITitle,tel) {
    $.ajax({
        data: tel,
        method: 'get',
        url: APITitle + 'api/Regist_Login/GetInformation?Phone=' + tel,//地区API
